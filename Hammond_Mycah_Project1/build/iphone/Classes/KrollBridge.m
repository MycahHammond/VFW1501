...
NSString * Hammond_Mycah_Project1$ModuleRequireFormat = @"(function(exports){"
...
@implementation Hammond_Mycah_Project1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_hammond_mycah_project1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Hammond_Mycah_Project1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Hammond_Mycah_Project1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_hammond_mycah_project1 = [[Hammond_Mycah_Project1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_hammond_mycah_project1];
	
	NSString *_hammond_mycah_project1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _hammond_mycah_project1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_hammond_mycah_project1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Hammond_Mycah_Project1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.hammond_mycah_project1.kroll" 
