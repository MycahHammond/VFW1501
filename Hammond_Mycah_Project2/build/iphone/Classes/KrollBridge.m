...
NSString * Hammond_Mycah_Project2$ModuleRequireFormat = @"(function(exports){"
...
@implementation Hammond_Mycah_Project2Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_hammond_mycah_project2);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Hammond_Mycah_Project2 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Hammond_Mycah_Project2" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_hammond_mycah_project2 = [[Hammond_Mycah_Project2Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_hammond_mycah_project2];
	
	NSString *_hammond_mycah_project2NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _hammond_mycah_project2NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_hammond_mycah_project2 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Hammond_Mycah_Project2$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.hammond_mycah_project2.kroll" 
